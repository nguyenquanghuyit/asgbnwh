...
/**
 * Table class for foxconn_sumbycode
 */
class foxconn_sumbycode extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'foxconn_sumbycode';
		$this->TableName = 'foxconn_sumbycode';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`foxconn_sumbycode`";
		$this->Dbid = 'DB';
		...
		// Code
		$this->Code = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->Sortable = TRUE; // Allow sort
		$this->fields['Code'] = &$this->Code;

		// ProductName
		$this->ProductName = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// Model
		$this->Model = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_Model', 'Model', '`Model`', '`Model`', 200, -1, FALSE, '`Model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Model->Sortable = TRUE; // Allow sort
		$this->fields['Model'] = &$this->Model;

		// SUM(ts.PCS)
		$this->SUM28ts_PCS29 = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_SUM28ts_PCS29', 'SUM(ts.PCS)', '`SUM(ts.PCS)`', '`SUM(ts.PCS)`', 131, -1, FALSE, '`SUM(ts.PCS)`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Pallet
		$this->Pallet = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_Pallet', 'Pallet', '`Pallet`', '`Pallet`', 3, -1, FALSE, '`Pallet`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->Pallet->Lookup = new Lookup('Pallet', 'foxconn_sumbycode', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
		$this->Pallet->OptionCount = 2;
		$this->Pallet->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['Pallet'] = &$this->Pallet;

		// TypeName
		$this->TypeName = new DbField('foxconn_sumbycode', 'foxconn_sumbycode', 'x_TypeName', 'TypeName', '`TypeName`', '`TypeName`', 200, -1, FALSE, '`TypeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`foxconn_sumbycode`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "foxconn_sumbycodelist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "foxconn_sumbycodeview.php")
			return $Language->phrase("View");
		elseif ($pageName == "foxconn_sumbycodeedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "foxconn_sumbycodeadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("foxconn_sumbycodeview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("foxconn_sumbycodeview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "foxconn_sumbycodeadd.php?" . $this->getUrlParm($parm);
		else
			$url = "foxconn_sumbycodeadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("foxconn_sumbycodeedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("foxconn_sumbycodeadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("foxconn_sumbycodedelete.php", $this->getUrlParm());
	}
