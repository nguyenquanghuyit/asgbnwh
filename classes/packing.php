...
/**
 * Table class for packing
 */
class packing extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'packing';
		$this->TableName = 'packing';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`packing`";
		$this->Dbid = 'DB';
		...
		// Id_packing
		$this->Id_packing = new DbField('packing', 'packing', 'x_Id_packing', 'Id_packing', '`Id_packing`', '`Id_packing`', 20, -1, FALSE, '`Id_packing`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Order
		$this->ID_Order = new DbField('packing', 'packing', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 3, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// CusomterOrderNo
		$this->CusomterOrderNo = new DbField('packing', 'packing', 'x_CusomterOrderNo', 'CusomterOrderNo', '`CusomterOrderNo`', '`CusomterOrderNo`', 200, -1, FALSE, '`CusomterOrderNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CusomterOrderNo->Sortable = TRUE; // Allow sort
		$this->fields['CusomterOrderNo'] = &$this->CusomterOrderNo;

		// TypeName
		$this->TypeName = new DbField('packing', 'packing', 'x_TypeName', 'TypeName', '`TypeName`', '`TypeName`', 200, -1, FALSE, '`TypeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->TypeName->Lookup = new Lookup('TypeName', 'packing', TRUE, 'TypeName', ["TypeName","","",""], [], [], [], [], [], [], '', '');
		$this->fields['TypeName'] = &$this->TypeName;

		// CreateUser
		$this->CreateUser = new DbField('packing', 'packing', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// CreateDate
		$this->CreateDate = new DbField('packing', 'packing', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 7, "DB"), 135, 7, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "tbl_packingdetail") {
			$detailUrl = $GLOBALS["tbl_packingdetail"]->getListUrl() . "?" . TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$detailUrl .= "&fk_Id_packing=" . urlencode($this->Id_packing->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "packinglist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`packing`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "packingedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "packingadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("packingview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("packingview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "packingadd.php?" . $this->getUrlParm($parm);
		else
			$url = "packingadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("packingedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("packingedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("packingadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("packingadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("packingdelete.php", $this->getUrlParm());
	}
