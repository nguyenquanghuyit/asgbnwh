...
/**
 * Table class for tbl_loading
 */
class tbl_loading extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'tbl_loading';
		$this->TableName = 'tbl_loading';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_loading`";
		$this->Dbid = 'DB';
		...
		// ID_Loading
		$this->ID_Loading = new DbField('tbl_loading', 'tbl_loading', 'x_ID_Loading', 'ID_Loading', '`ID_Loading`', '`ID_Loading`', 3, -1, FALSE, '`ID_Loading`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID_Loading->IsAutoIncrement = TRUE; // Autoincrement field
		$this->ID_Loading->IsPrimaryKey = TRUE; // Primary key field
		$this->ID_Loading->Sortable = FALSE; // Allow sort
		$this->ID_Loading->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ID_Loading'] = &$this->ID_Loading;

		// ID_Route
		$this->ID_Route = new DbField('tbl_loading', 'tbl_loading', 'x_ID_Route', 'ID_Route', '`ID_Route`', '`ID_Route`', 3, -1, FALSE, '`ID_Route`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Order
		$this->ID_Order = new DbField('tbl_loading', 'tbl_loading', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 3, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->ID_Order->Sortable = TRUE; // Allow sort
		$this->ID_Order->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->ID_Order->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		$this->ID_Order->Lookup = new Lookup('ID_Order', 'vworder_not_route', FALSE, 'ID_Order', ["ID_Order","","",""], [], [], [], [], ["CusomterOrderNo"], ["x_CusomterOrderNo"], '', '');
		$this->ID_Order->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ID_Order'] = &$this->ID_Order;

		// CusomterOrderNo
		$this->CusomterOrderNo = new DbField('tbl_loading', 'tbl_loading', 'x_CusomterOrderNo', 'CusomterOrderNo', '`CusomterOrderNo`', '`CusomterOrderNo`', 200, -1, FALSE, '`CusomterOrderNo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CusomterOrderNo->Sortable = TRUE; // Allow sort
		$this->fields['CusomterOrderNo'] = &$this->CusomterOrderNo;

		// CreateUser
		$this->CreateUser = new DbField('tbl_loading', 'tbl_loading', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// CreateDate
		$this->CreateDate = new DbField('tbl_loading', 'tbl_loading', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 0, "DB"), 135, 0, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// UpdateUser
		$this->UpdateUser = new DbField('tbl_loading', 'tbl_loading', 'x_UpdateUser', 'UpdateUser', '`UpdateUser`', '`UpdateUser`', 200, -1, FALSE, '`UpdateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UpdateUser->Sortable = TRUE; // Allow sort
		$this->fields['UpdateUser'] = &$this->UpdateUser;

		// UpdateDate
		$this->UpdateDate = new DbField('tbl_loading', 'tbl_loading', 'x_UpdateDate', 'UpdateDate', '`UpdateDate`', CastDateFieldForLike('`UpdateDate`', 0, "DB"), 135, 0, FALSE, '`UpdateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`tbl_loading`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->ID_Loading->setDbValue($conn->insert_ID());
			$rs['ID_Loading'] = $this->ID_Loading->DbValue;
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ID_Loading', $rs))
				AddFilter($where, QuotedName('ID_Loading', $this->Dbid) . '=' . QuotedValue($rs['ID_Loading'], $this->ID_Loading->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->ID_Loading->DbValue = $row['ID_Loading'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('ID_Loading', $row) ? $row['ID_Loading'] : NULL) : $this->ID_Loading->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@ID_Loading@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "tbl_loadingview.php")
			return $Language->phrase("View");
		elseif ($pageName == "tbl_loadingedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tbl_loadingadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_loadingview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_loadingview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "tbl_loadingadd.php?" . $this->getUrlParm($parm);
		else
			$url = "tbl_loadingadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_loadingedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_loadingadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_loadingdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "ID_Loading:" . JsonEncode($this->ID_Loading->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->ID_Loading->CurrentValue != NULL) {
			$url .= "ID_Loading=" . urlencode($this->ID_Loading->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("ID_Loading") !== NULL)
				$arKeys[] = Param("ID_Loading");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->ID_Loading->CurrentValue = $key;
	...
	{
		$this->ID_Loading->setDbValue($rs->fields('ID_Loading'));
	...
	public function renderListRow()
	{
		...
		// ID_Loading
		$this->ID_Loading->ViewValue = $this->ID_Loading->CurrentValue;
		$this->ID_Loading->ViewCustomAttributes = "";
		...
		// ID_Loading
		$this->ID_Loading->LinkCustomAttributes = "";
		$this->ID_Loading->HrefValue = "";
		$this->ID_Loading->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// ID_Loading
		$this->ID_Loading->EditAttrs["class"] = "form-control";
		$this->ID_Loading->EditCustomAttributes = "";
		$this->ID_Loading->EditValue = $this->ID_Loading->CurrentValue;
		$this->ID_Loading->ViewCustomAttributes = "";
