...
/**
 * Table class for tbl_location
 */
class tbl_location extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'tbl_location';
		$this->TableName = 'tbl_location';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_location`";
		$this->Dbid = 'DB';
		...
		// ID_Location
		$this->ID_Location = new DbField('tbl_location', 'tbl_location', 'x_ID_Location', 'ID_Location', '`ID_Location`', '`ID_Location`', 3, -1, FALSE, '`ID_Location`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID_Location->IsAutoIncrement = TRUE; // Autoincrement field
		$this->ID_Location->IsPrimaryKey = TRUE; // Primary key field
		...
		// Location
		$this->Location = new DbField('tbl_location', 'tbl_location', 'x_Location', 'Location', '`Location`', '`Location`', 200, -1, FALSE, '`Location`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Location->Required = TRUE; // Required field
		$this->Location->Sortable = TRUE; // Allow sort
		$this->fields['Location'] = &$this->Location;

		// Description
		$this->Description = new DbField('tbl_location', 'tbl_location', 'x_Description', 'Description', '`Description`', '`Description`', 200, -1, FALSE, '`Description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Description->Sortable = TRUE; // Allow sort
		$this->fields['Description'] = &$this->Description;

		// CreateUser
		$this->CreateUser = new DbField('tbl_location', 'tbl_location', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// CreateDate
		$this->CreateDate = new DbField('tbl_location', 'tbl_location', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 11, "DB"), 135, 11, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// UpdateUser
		$this->UpdateUser = new DbField('tbl_location', 'tbl_location', 'x_UpdateUser', 'UpdateUser', '`UpdateUser`', '`UpdateUser`', 200, -1, FALSE, '`UpdateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UpdateUser->Sortable = FALSE; // Allow sort
		$this->fields['UpdateUser'] = &$this->UpdateUser;

		// UpdateDate
		$this->UpdateDate = new DbField('tbl_location', 'tbl_location', 'x_UpdateDate', 'UpdateDate', '`UpdateDate`', CastDateFieldForLike('`UpdateDate`', 11, "DB"), 135, 11, FALSE, '`UpdateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`tbl_location`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->ID_Location->setDbValue($conn->insert_ID());
			$rs['ID_Location'] = $this->ID_Location->DbValue;
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ID_Location', $rs))
				AddFilter($where, QuotedName('ID_Location', $this->Dbid) . '=' . QuotedValue($rs['ID_Location'], $this->ID_Location->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('ID_Location', $row) ? $row['ID_Location'] : NULL) : $this->ID_Location->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@ID_Location@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "tbl_locationview.php")
			return $Language->phrase("View");
		elseif ($pageName == "tbl_locationedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tbl_locationadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_locationview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_locationview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "tbl_locationadd.php?" . $this->getUrlParm($parm);
		else
			$url = "tbl_locationadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_locationedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_locationadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_locationdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "ID_Location:" . JsonEncode($this->ID_Location->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->ID_Location->CurrentValue != NULL) {
			$url .= "ID_Location=" . urlencode($this->ID_Location->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("ID_Location") !== NULL)
				$arKeys[] = Param("ID_Location");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->ID_Location->CurrentValue = $key;
