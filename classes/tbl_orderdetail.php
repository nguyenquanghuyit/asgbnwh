...
/**
 * Table class for tbl_orderdetail
 */
class tbl_orderdetail extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'tbl_orderdetail';
		$this->TableName = 'tbl_orderdetail';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_orderdetail`";
		$this->Dbid = 'DB';
		...
		// ID_Detail
		$this->ID_Detail = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_ID_Detail', 'ID_Detail', '`ID_Detail`', '`ID_Detail`', 3, -1, FALSE, '`ID_Detail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID_Detail->IsAutoIncrement = TRUE; // Autoincrement field
		$this->ID_Detail->IsPrimaryKey = TRUE; // Primary key field
		$this->ID_Detail->Sortable = FALSE; // Allow sort
		$this->ID_Detail->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ID_Detail'] = &$this->ID_Detail;

		// Code
		$this->Code = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`EV__Code`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->Required = TRUE; // Required field
		$this->Code->Sortable = TRUE; // Allow sort
		$this->Code->Lookup = new Lookup('Code', 'vwcodeinstock', FALSE, 'Code', ["Code","ProductName","",""], [], [], [], [], ["PCS"], ["x_PCS"], '', '');
		$this->fields['Code'] = &$this->Code;

		// PCS
		$this->PCS = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_PCS', 'PCS', '`PCS`', '`PCS`', 3, -1, FALSE, '`PCS`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_Order
		$this->ID_Order = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 3, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ID_Order->IsForeignKey = TRUE; // Foreign key field
		$this->ID_Order->Nullable = FALSE; // NOT NULL field
		$this->ID_Order->Required = TRUE; // Required field
		...
		// StatusPickUp
		$this->StatusPickUp = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_StatusPickUp', 'StatusPickUp', '`StatusPickUp`', '`StatusPickUp`', 16, -1, FALSE, '`StatusPickUp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->StatusPickUp->Sortable = FALSE; // Allow sort
		$this->StatusPickUp->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->StatusPickUp->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		$this->StatusPickUp->Lookup = new Lookup('StatusPickUp', 'tbl_orderdetail', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
		$this->StatusPickUp->OptionCount = 2;
		$this->StatusPickUp->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['StatusPickUp'] = &$this->StatusPickUp;

		// CreateUser
		$this->CreateUser = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// CreateDate
		$this->CreateDate = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 0, "DB"), 135, 0, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// UpdateUser
		$this->UpdateUser = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_UpdateUser', 'UpdateUser', '`UpdateUser`', '`UpdateUser`', 200, -1, FALSE, '`UpdateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UpdateUser->Sortable = TRUE; // Allow sort
		$this->fields['UpdateUser'] = &$this->UpdateUser;

		// UpdateDate
		$this->UpdateDate = new DbField('tbl_orderdetail', 'tbl_orderdetail', 'x_UpdateDate', 'UpdateDate', '`UpdateDate`', CastDateFieldForLike('`UpdateDate`', 0, "DB"), 135, 0, FALSE, '`UpdateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`tbl_orderdetail`";
	}
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`Code`, ''),'" . ValueSeparator(1, $this->Code) . "',COALESCE(`ProductName`,'')) FROM `vwcodeinstock` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`Code` = `tbl_orderdetail`.`Code` LIMIT 1) AS `EV__Code` FROM `tbl_orderdetail`" .
			") `TMP_TABLE`";
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->ID_Detail->setDbValue($conn->insert_ID());
			$rs['ID_Detail'] = $this->ID_Detail->DbValue;
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ID_Detail', $rs))
				AddFilter($where, QuotedName('ID_Detail', $this->Dbid) . '=' . QuotedValue($rs['ID_Detail'], $this->ID_Detail->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->StatusPickUp->DbValue = $row['StatusPickUp'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('ID_Detail', $row) ? $row['ID_Detail'] : NULL) : $this->ID_Detail->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@ID_Detail@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "tbl_orderdetaillist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "tbl_orderdetailview.php")
			return $Language->phrase("View");
		elseif ($pageName == "tbl_orderdetailedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tbl_orderdetailadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_orderdetailview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_orderdetailview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "tbl_orderdetailadd.php?" . $this->getUrlParm($parm);
		else
			$url = "tbl_orderdetailadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_orderdetailedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_orderdetailadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_orderdetaildelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "ID_Detail:" . JsonEncode($this->ID_Detail->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->ID_Detail->CurrentValue != NULL) {
			$url .= "ID_Detail=" . urlencode($this->ID_Detail->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("ID_Detail") !== NULL)
				$arKeys[] = Param("ID_Detail");
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->StatusPickUp->setDbValue($rs->fields('StatusPickUp'));
	...
	public function renderEditRow()
	{
		...
		if (strval($this->StatusPickUp->CurrentValue) <> "") {
			$this->StatusPickUp->EditValue = $this->StatusPickUp->optionCaption($this->StatusPickUp->CurrentValue);
		} else {
			$this->StatusPickUp->EditValue = NULL;
		}
		$this->StatusPickUp->CellCssStyle .= "text-align: center;";
		$this->StatusPickUp->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->StatusPickUp);
				} else {
					$doc->exportCaption($this->Code);
					$doc->exportCaption($this->PCS);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->StatusPickUp);
					} else {
						$doc->exportField($this->Code);
						$doc->exportField($this->PCS);
