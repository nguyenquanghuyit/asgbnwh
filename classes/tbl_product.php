 ...
 */
class tbl_product extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// IdType
		$this->IdType = new DbField('tbl_product', 'tbl_product', 'x_IdType', 'IdType', '`IdType`', '`IdType`', 200, -1, FALSE, '`IdType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Code
		$this->Code = new DbField('tbl_product', 'tbl_product', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Model
		$this->Model = new DbField('tbl_product', 'tbl_product', 'x_Model', 'Model', '`Model`', '`Model`', 200, -1, FALSE, '`Model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Model->Sortable = TRUE; // Allow sort
		$this->fields['Model'] = &$this->Model;

		// ProductName
		$this->ProductName = new DbField('tbl_product', 'tbl_product', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// VnName
		$this->VnName = new DbField('tbl_product', 'tbl_product', 'x_VnName', 'VnName', '`VnName`', '`VnName`', 200, -1, FALSE, '`VnName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->VnName->Sortable = TRUE; // Allow sort
		$this->fields['VnName'] = &$this->VnName;

		// ID_Contact
		$this->ID_Contact = new DbField('tbl_product', 'tbl_product', 'x_ID_Contact', 'ID_Contact', '`ID_Contact`', '`ID_Contact`', 3, -1, FALSE, '`EV__ID_Contact`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// Description
		$this->Description = new DbField('tbl_product', 'tbl_product', 'x_Description', 'Description', '`Description`', '`Description`', 200, -1, FALSE, '`Description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Description->Sortable = TRUE; // Allow sort
		$this->fields['Description'] = &$this->Description;

		// CreateDate
		$this->CreateDate = new DbField('tbl_product', 'tbl_product', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 11, "DB"), 135, 11, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CreateUser
		$this->CreateUser = new DbField('tbl_product', 'tbl_product', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// UpdateUser
		$this->UpdateUser = new DbField('tbl_product', 'tbl_product', 'x_UpdateUser', 'UpdateUser', '`UpdateUser`', '`UpdateUser`', 200, -1, FALSE, '`UpdateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->UpdateUser->Sortable = TRUE; // Allow sort
		$this->fields['UpdateUser'] = &$this->UpdateUser;

		// UpdateDate
		$this->UpdateDate = new DbField('tbl_product', 'tbl_product', 'x_UpdateDate', 'UpdateDate', '`UpdateDate`', CastDateFieldForLike('`UpdateDate`', 11, "DB"), 135, 11, FALSE, '`UpdateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "tbl_product_detail") {
			$detailUrl = $GLOBALS["tbl_product_detail"]->getListUrl() . "?" . TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$detailUrl .= "&fk_Code=" . urlencode($this->Code->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "tbl_productlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`tbl_product`";
	}
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT CONCAT(COALESCE(`CodeContact`, ''),'" . ValueSeparator(1, $this->ID_Contact) . "',COALESCE(`ContactName`,'')) FROM `tbl_contact` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`ID_Contact` = `tbl_product`.`ID_Contact` LIMIT 1) AS `EV__ID_Contact` FROM `tbl_product`" .
			") `TMP_TABLE`";
	...
	public function update(&$rs, $where = "", $rsold = NULL, $curfilter = TRUE)
	{
		...
		// Cascade Update detail table 'tbl_product_detail'
		$cascadeUpdate = FALSE;
		$rscascade = array();
		if ($rsold && (isset($rs['Code']) && $rsold['Code'] <> $rs['Code'])) { // Update detail field 'CODE'
			$cascadeUpdate = TRUE;
			$rscascade['CODE'] = $rs['Code']; 
		}
		if ($cascadeUpdate) {
			if (!isset($GLOBALS["tbl_product_detail"]))
				$GLOBALS["tbl_product_detail"] = new tbl_product_detail();
			$rswrk = $GLOBALS["tbl_product_detail"]->loadRs("`CODE` = " . QuotedValue($rsold['Code'], DATATYPE_STRING, 'DB')); 
			while ($rswrk && !$rswrk->EOF) {
				...
				$success = $GLOBALS["tbl_product_detail"]->Row_Updating($rsdtlold, $rsdtlnew);
				if ($success)
					$success = $GLOBALS["tbl_product_detail"]->update($rscascade, $rskey, $rswrk->fields);
				if (!$success)
					return FALSE;

				// Call Row_Updated event
				$GLOBALS["tbl_product_detail"]->Row_Updated($rsdtlold, $rsdtlnew);
				$rswrk->moveNext();
			}
		}
	...
	public function delete(&$rs, $where = "", $curfilter = FALSE)
	{
		...
		// Cascade delete detail table 'tbl_product_detail'
		if (!isset($GLOBALS["tbl_product_detail"]))
			$GLOBALS["tbl_product_detail"] = new tbl_product_detail();
		$rscascade = $GLOBALS["tbl_product_detail"]->loadRs("`CODE` = " . QuotedValue($rs['Code'], DATATYPE_STRING, "DB")); 
		$dtlrows = ($rscascade) ? $rscascade->getRows() : array();

		// Call Row Deleting event
		foreach ($dtlrows as $dtlrow) {
			$success = $GLOBALS["tbl_product_detail"]->Row_Deleting($dtlrow);
			if (!$success)
				break;
		}
		if ($success) {
			foreach ($dtlrows as $dtlrow) {
				$success = $GLOBALS["tbl_product_detail"]->delete($dtlrow); // Delete
				if (!$success)
					break;
			}
		}
		...
		if ($success) {
			foreach ($dtlrows as $dtlrow)
				$GLOBALS["tbl_product_detail"]->Row_Deleted($dtlrow);
		}
		if ($success)
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_productview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_productview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "tbl_productadd.php?" . $this->getUrlParm($parm);
		else
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_productedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_productedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tbl_productadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_productadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_productdelete.php", $this->getUrlParm());
	}
