...
/**
 * Table class for vwRouteUnload
 */
class vwRouteUnload extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwRouteUnload';
		$this->TableName = 'vwRouteUnload';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwRouteUnload`";
		$this->Dbid = 'DB';
		...
		// ID_Route
		$this->ID_Route = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_ID_Route', 'ID_Route', '`ID_Route`', '`ID_Route`', 3, -1, FALSE, '`ID_Route`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// PalletID
		$this->PalletID = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_PalletID', 'PalletID', '`PalletID`', '`PalletID`', 200, -1, FALSE, '`PalletID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PalletID->Sortable = TRUE; // Allow sort
		$this->fields['PalletID'] = &$this->PalletID;

		// Code
		$this->Code = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PCS
		$this->PCS = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_PCS', 'PCS', '`PCS`', '`PCS`', 3, -1, FALSE, '`PCS`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// PcsReal
		$this->PcsReal = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_PcsReal', 'PcsReal', '`PcsReal`', '`PcsReal`', 3, -1, FALSE, '`PcsReal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PcsReal->Nullable = FALSE; // NOT NULL field
		$this->PcsReal->Required = TRUE; // Required field
		$this->PcsReal->Sortable = TRUE; // Allow sort
		$this->PcsReal->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['PcsReal'] = &$this->PcsReal;

		// CreateUser
		$this->CreateUser = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CreateUser->Sortable = TRUE; // Allow sort
		$this->fields['CreateUser'] = &$this->CreateUser;

		// CreateDate
		$this->CreateDate = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 0, "DB"), 135, 0, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// MFG
		$this->MFG = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_MFG', 'MFG', '`MFG`', CastDateFieldForLike('`MFG`', 0, "DB"), 133, 0, FALSE, '`MFG`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->MFG->Sortable = TRUE; // Allow sort
		$this->MFG->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['MFG'] = &$this->MFG;

		// Description
		$this->Description = new DbField('vwRouteUnload', 'vwRouteUnload', 'x_Description', 'Description', '`Description`', '`Description`', 200, -1, FALSE, '`Description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwRouteUnload`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->PcsReal->DbValue = $row['PcsReal'];
	...
	{
		global $Language;
		if ($pageName == "vwRouteUnloadview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vwRouteUnloadedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwRouteUnloadadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwRouteUnloadview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwRouteUnloadview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwRouteUnloadadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwRouteUnloadadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwRouteUnloadedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwRouteUnloadadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwRouteUnloaddelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->PcsReal->setDbValue($rs->fields('PcsReal'));
	...
	public function renderEditRow()
	{
		...
		// PcsReal
		$this->PcsReal->EditAttrs["class"] = "form-control";
		$this->PcsReal->EditCustomAttributes = "";
		$this->PcsReal->EditValue = $this->PcsReal->CurrentValue;
		$this->PcsReal->PlaceHolder = RemoveHtml($this->PcsReal->caption());
		...
		$this->MFG->EditValue = FormatDateTime($this->MFG->CurrentValue, 8);
	...
	public function exportDocument($doc, $recordset, $startRec = 1, $stopRec = 1, $exportPageType = "")
	{
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->PcsReal);
		...
		if ($doc->Horizontal) {
			...
			if (!$doc->ExportCustom) {
				...
				$doc->exportAggregate($this->PcsReal, 'TOTAL');
