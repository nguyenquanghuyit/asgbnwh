...
/**
 * Table class for vwbroker
 */
class vwbroker extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwbroker';
		$this->TableName = 'vwbroker';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwbroker`";
		$this->Dbid = 'DB';
		...
		// OrderID
		$this->OrderID = new DbField('vwbroker', 'vwbroker', 'x_OrderID', 'OrderID', '`OrderID`', '`OrderID`', 3, -1, FALSE, '`OrderID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// ID_Order
		$this->ID_Order = new DbField('vwbroker', 'vwbroker', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 200, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// CodeContact
		$this->CodeContact = new DbField('vwbroker', 'vwbroker', 'x_CodeContact', 'CodeContact', '`CodeContact`', '`CodeContact`', 200, -1, FALSE, '`CodeContact`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CodeContact->Sortable = TRUE; // Allow sort
		$this->fields['CodeContact'] = &$this->CodeContact;

		// CompanyName
		$this->CompanyName = new DbField('vwbroker', 'vwbroker', 'x_CompanyName', 'CompanyName', '`CompanyName`', '`CompanyName`', 200, -1, FALSE, '`CompanyName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CompanyName->Sortable = TRUE; // Allow sort
		$this->fields['CompanyName'] = &$this->CompanyName;

		// Address
		$this->Address = new DbField('vwbroker', 'vwbroker', 'x_Address', 'Address', '`Address`', '`Address`', 200, -1, FALSE, '`Address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Address->Sortable = TRUE; // Allow sort
		$this->fields['Address'] = &$this->Address;

		// TypeName
		$this->TypeName = new DbField('vwbroker', 'vwbroker', 'x_TypeName', 'TypeName', '`TypeName`', '`TypeName`', 200, -1, FALSE, '`TypeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->TypeName->Lookup = new Lookup('TypeName', 'vwbroker', TRUE, 'TypeName', ["TypeName","","",""], [], [], [], [], [], [], '', '');
		$this->fields['TypeName'] = &$this->TypeName;

		// Code
		$this->Code = new DbField('vwbroker', 'vwbroker', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->IsPrimaryKey = TRUE; // Primary key field
		$this->Code->Sortable = TRUE; // Allow sort
		$this->fields['Code'] = &$this->Code;

		// ProductName
		$this->ProductName = new DbField('vwbroker', 'vwbroker', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// Model
		$this->Model = new DbField('vwbroker', 'vwbroker', 'x_Model', 'Model', '`Model`', '`Model`', 200, -1, FALSE, '`Model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Model->Sortable = TRUE; // Allow sort
		$this->fields['Model'] = &$this->Model;

		// PCS
		$this->PCS = new DbField('vwbroker', 'vwbroker', 'x_PCS', 'PCS', '`PCS`', '`PCS`', 3, -1, FALSE, '`PCS`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PCS->Sortable = TRUE; // Allow sort
		$this->PCS->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['PCS'] = &$this->PCS;

		// CreateDate
		$this->CreateDate = new DbField('vwbroker', 'vwbroker', 'x_CreateDate', 'CreateDate', '`CreateDate`', CastDateFieldForLike('`CreateDate`', 11, "DB"), 135, 11, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// OrderDate
		$this->OrderDate = new DbField('vwbroker', 'vwbroker', 'x_OrderDate', 'OrderDate', '`OrderDate`', CastDateFieldForLike('`OrderDate`', 1, "DB"), 135, 1, FALSE, '`OrderDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwbroker`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`CreateDate` DESC,`ID_Order` DESC,`Code` ASC";
	}
	...
	{
		return "`ID_Order` = '@ID_Order@' AND `Code` = '@Code@'";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "vwbrokeredit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwbrokeradd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwbrokerview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwbrokerview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwbrokeradd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwbrokeradd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwbrokeredit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwbrokeradd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwbrokerdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "ID_Order:" . JsonEncode($this->ID_Order->CurrentValue, "string");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->ID_Order->CurrentValue = $key[0];
	...
	public function exportDocument($doc, $recordset, $startRec = 1, $stopRec = 1, $exportPageType = "")
	{
		...
		if ($doc->Horizontal) {
			...
			if (!$doc->ExportCustom) {
				...
				$doc->exportAggregate($this->CodeContact, '');
				...
				$doc->exportAggregate($this->OrderDate, '');
				$doc->endExportRow();
			}
		}
		if (!$doc->ExportCustom) {
