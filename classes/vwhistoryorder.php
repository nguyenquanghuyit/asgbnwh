...
/**
 * Table class for vwhistoryorder
 */
class vwhistoryorder extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwhistoryorder';
		$this->TableName = 'vwhistoryorder';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwhistoryorder`";
		$this->Dbid = 'DB';
		...
		// ID_Order
		$this->ID_Order = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 3, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID_His
		$this->ID_His = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_ID_His', 'ID_His', '`ID_His`', '`ID_His`', 3, -1, FALSE, '`ID_His`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ID_His->IsAutoIncrement = TRUE; // Autoincrement field
		$this->ID_His->IsPrimaryKey = TRUE; // Primary key field
		$this->ID_His->Sortable = TRUE; // Allow sort
		$this->ID_His->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['ID_His'] = &$this->ID_His;

		// PalletID
		$this->PalletID = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_PalletID', 'PalletID', '`PalletID`', '`PalletID`', 200, -1, FALSE, '`PalletID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PalletID->Sortable = TRUE; // Allow sort
		$this->fields['PalletID'] = &$this->PalletID;

		// Code
		$this->Code = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->Sortable = TRUE; // Allow sort
		$this->fields['Code'] = &$this->Code;

		// ProductName
		$this->ProductName = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// PCS_Out
		$this->PCS_Out = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_PCS_Out', 'PCS_Out', '`PCS_Out`', '`PCS_Out`', 3, -1, FALSE, '`PCS_Out`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateIn
		$this->DateIn = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_DateIn', 'DateIn', '`DateIn`', CastDateFieldForLike('`DateIn`', 11, "DB"), 135, 11, FALSE, '`DateIn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// DateOut
		$this->DateOut = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_DateOut', 'DateOut', '`DateOut`', CastDateFieldForLike('`DateOut`', 11, "DB"), 135, 11, FALSE, '`DateOut`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->DateOut->Sortable = TRUE; // Allow sort
		$this->DateOut->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_SEPARATOR"], $Language->phrase("IncorrectDateDMY"));
		$this->fields['DateOut'] = &$this->DateOut;

		// PalletStatus
		$this->PalletStatus = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_PalletStatus', 'PalletStatus', '`PalletStatus`', '`PalletStatus`', 200, -1, FALSE, '`PalletStatus`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->PalletStatus->Sortable = TRUE; // Allow sort
		$this->fields['PalletStatus'] = &$this->PalletStatus;

		// CreateUser
		$this->CreateUser = new DbField('vwhistoryorder', 'vwhistoryorder', 'x_CreateUser', 'CreateUser', '`CreateUser`', '`CreateUser`', 200, -1, FALSE, '`CreateUser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwhistoryorder`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`DateOut` ASC";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->DateOut->DbValue = $row['DateOut'];
	...
	{
		global $Language;
		if ($pageName == "vwhistoryorderview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vwhistoryorderedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwhistoryorderadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwhistoryorderview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwhistoryorderview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwhistoryorderadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwhistoryorderadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwhistoryorderedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwhistoryorderadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwhistoryorderdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->DateOut->setDbValue($rs->fields('DateOut'));
	...
	public function renderEditRow()
	{
		...
		// DateOut
		$this->DateOut->EditAttrs["class"] = "form-control";
		$this->DateOut->EditCustomAttributes = "";
		$this->DateOut->EditValue = FormatDateTime($this->DateOut->CurrentValue, 11);
		$this->DateOut->PlaceHolder = RemoveHtml($this->DateOut->caption());
	...
	public function exportDocument($doc, $recordset, $startRec = 1, $stopRec = 1, $exportPageType = "")
	{
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->DateOut);
		...
		if ($doc->Horizontal) {
			...
			if (!$doc->ExportCustom) {
				...
				$doc->exportAggregate($this->ID_Order, '');
				$doc->exportAggregate($this->ID_His, '');
				...
				$doc->exportAggregate($this->DateOut, '');
