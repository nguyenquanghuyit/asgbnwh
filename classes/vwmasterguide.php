...
/**
 * Table class for vwmasterguide
 */
class vwmasterguide extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwmasterguide';
		$this->TableName = 'vwmasterguide';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwmasterguide`";
		$this->Dbid = 'DB';
		...
		// ID_Order
		$this->ID_Order = new DbField('vwmasterguide', 'vwmasterguide', 'x_ID_Order', 'ID_Order', '`ID_Order`', '`ID_Order`', 3, -1, FALSE, '`ID_Order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "tbl_guider") {
			$detailUrl = $GLOBALS["tbl_guider"]->getListUrl() . "?" . TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$detailUrl .= "&fk_ID_Order=" . urlencode($this->ID_Order->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "vwmasterguidelist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwmasterguide`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "vwmasterguideedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwmasterguideadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwmasterguideview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwmasterguideview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwmasterguideadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwmasterguideadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwmasterguideedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwmasterguideedit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwmasterguideadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwmasterguideadd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwmasterguidedelete.php", $this->getUrlParm());
	}
