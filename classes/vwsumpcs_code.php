...
/**
 * Table class for vwsumpcs_code
 */
class vwsumpcs_code extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwsumpcs_code';
		$this->TableName = 'vwsumpcs_code';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwsumpcs_code`";
		$this->Dbid = 'DB';
		...
		// TypeName
		$this->TypeName = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_TypeName', 'TypeName', '`TypeName`', '`TypeName`', 200, -1, FALSE, '`TypeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->TypeName->Lookup = new Lookup('TypeName', 'vwsumpcs_code', TRUE, 'TypeName', ["TypeName","","",""], [], [], [], [], [], [], '', '');
		$this->fields['TypeName'] = &$this->TypeName;

		// Code
		$this->Code = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->IsPrimaryKey = TRUE; // Primary key field
		$this->Code->Sortable = TRUE; // Allow sort
		$this->fields['Code'] = &$this->Code;

		// ProductName
		$this->ProductName = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// Model
		$this->Model = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_Model', 'Model', '`Model`', '`Model`', 200, -1, FALSE, '`Model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Model->Sortable = TRUE; // Allow sort
		$this->fields['Model'] = &$this->Model;

		// SUM(ts.PCS)
		$this->SUM28ts_PCS29 = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_SUM28ts_PCS29', 'SUM(ts.PCS)', '`SUM(ts.PCS)`', '`SUM(ts.PCS)`', 131, -1, FALSE, '`SUM(ts.PCS)`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Pallet
		$this->Pallet = new DbField('vwsumpcs_code', 'vwsumpcs_code', 'x_Pallet', 'Pallet', '`Pallet`', '`Pallet`', 3, -1, FALSE, '`Pallet`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->Pallet->IsPrimaryKey = TRUE; // Primary key field
		...
		$this->Pallet->Lookup = new Lookup('Pallet', 'vwsumpcs_code', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwsumpcs_code`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`Code` ASC,`Pallet` ASC";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			...
			if (array_key_exists('Pallet', $rs))
				AddFilter($where, QuotedName('Pallet', $this->Dbid) . '=' . QuotedValue($rs['Pallet'], $this->Pallet->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	{
		return "`Code` = '@Code@' AND `Pallet` = @Pallet@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('Pallet', $row) ? $row['Pallet'] : NULL) : $this->Pallet->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@Pallet@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "vwsumpcs_codeview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vwsumpcs_codeedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwsumpcs_codeadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwsumpcs_codeview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwsumpcs_codeview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwsumpcs_codeadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwsumpcs_codeadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwsumpcs_codeedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwsumpcs_codeadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwsumpcs_codedelete.php", $this->getUrlParm());
	}
	...
	public function keyToJson($htmlEncode = FALSE)
	{
		...
		$json .= ",Pallet:" . JsonEncode($this->Pallet->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->Pallet->CurrentValue != NULL) {
			$url .= "&Pallet=" . urlencode($this->Pallet->CurrentValue);
		} else {
			return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
		}
		return $url;
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->Code->CurrentValue = $key[0];
			$this->Pallet->CurrentValue = $key[1];
	...
	public function renderEditRow()
	{
		...
		if (strval($this->Pallet->CurrentValue) <> "") {
			$this->Pallet->EditValue = $this->Pallet->optionCaption($this->Pallet->CurrentValue);
		} else {
			$this->Pallet->EditValue = NULL;
		}
		$this->Pallet->CellCssStyle .= "text-align: center;";
		$this->Pallet->ViewCustomAttributes = "";
