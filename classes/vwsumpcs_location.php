...
/**
 * Table class for vwsumpcs_location
 */
class vwsumpcs_location extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'vwsumpcs_location';
		$this->TableName = 'vwsumpcs_location';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`vwsumpcs_location`";
		$this->Dbid = 'DB';
		...
		// Location
		$this->Location = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_Location', 'Location', '`Location`', '`Location`', 200, -1, FALSE, '`Location`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Location->IsPrimaryKey = TRUE; // Primary key field
		$this->Location->Sortable = TRUE; // Allow sort
		$this->fields['Location'] = &$this->Location;

		// TypeName
		$this->TypeName = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_TypeName', 'TypeName', '`TypeName`', '`TypeName`', 200, -1, FALSE, '`TypeName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->TypeName->Lookup = new Lookup('TypeName', 'vwsumpcs_location', TRUE, 'TypeName', ["TypeName","","",""], [], [], [], [], [], [], '', '');
		$this->fields['TypeName'] = &$this->TypeName;

		// Code
		$this->Code = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_Code', 'Code', '`Code`', '`Code`', 200, -1, FALSE, '`Code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Code->IsPrimaryKey = TRUE; // Primary key field
		$this->Code->Sortable = TRUE; // Allow sort
		$this->fields['Code'] = &$this->Code;

		// ProductName
		$this->ProductName = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_ProductName', 'ProductName', '`ProductName`', '`ProductName`', 200, -1, FALSE, '`ProductName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ProductName->Sortable = TRUE; // Allow sort
		$this->fields['ProductName'] = &$this->ProductName;

		// Model
		$this->Model = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_Model', 'Model', '`Model`', '`Model`', 200, -1, FALSE, '`Model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Model->Sortable = TRUE; // Allow sort
		$this->fields['Model'] = &$this->Model;

		// PCS
		$this->PCS = new DbField('vwsumpcs_location', 'vwsumpcs_location', 'x_PCS', 'PCS', '`PCS`', '`PCS`', 131, -1, FALSE, '`PCS`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`vwsumpcs_location`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`Location` ASC,`Code` ASC";
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('Location', $rs))
				AddFilter($where, QuotedName('Location', $this->Dbid) . '=' . QuotedValue($rs['Location'], $this->Location->DataType, $this->Dbid));
	...
	{
		return "`Location` = '@Location@' AND `Code` = '@Code@'";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('Location', $row) ? $row['Location'] : NULL) : $this->Location->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@Location@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "vwsumpcs_locationlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "vwsumpcs_locationview.php")
			return $Language->phrase("View");
		elseif ($pageName == "vwsumpcs_locationedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "vwsumpcs_locationadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("vwsumpcs_locationview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("vwsumpcs_locationview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "vwsumpcs_locationadd.php?" . $this->getUrlParm($parm);
		else
			$url = "vwsumpcs_locationadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwsumpcs_locationedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("vwsumpcs_locationadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("vwsumpcs_locationdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "Location:" . JsonEncode($this->Location->CurrentValue, "string");
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->Location->CurrentValue != NULL) {
			$url .= "Location=" . urlencode($this->Location->CurrentValue);
		} else {
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->Location->CurrentValue = $key[0];
