...
function CurrentProjectID() {
	...
	return "{004E2122-9CCA-46AA-835C-0778C5A080BC}";
}
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$UserProfile->delete('pass'); // Delete password
	...
	public function loadUserID()
	{
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($rsUser = $UserTableConn->execute($sql)) {
				while (!$rsUser->EOF) {
					$this->addUserID($rsUser->fields('id_User'));
					$rsUser->moveNext();
				}
				$rsUser->close();
			}
