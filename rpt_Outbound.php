...
//header("Content-Disposition: attachment; filename=OUTBOUND_".date('d/m/Y').".xls");
?>
<head>
...
<table border=0 cellpadding=0 cellspacing=0 width=891 class=xl317014562
 style='border-collapse:collapse;width:669pt'>
...
">
   <v:imagedata src="rpt_Outbound_files/OUTBOUND_14562_image001.png" o:title=""/>
...
" o:insetmode="auto">
   <v:imagedata src="rpt_Outbound_files/OUTBOUND_14562_image002.png" o:title=""/>
...
" o:insetmode="auto">
    <v:imagedata src="rpt_Outbound_files/OUTBOUND_14562_image003.png" o:title=""/>
    <o:lock v:ext="edit" aspectratio="f"/>
    <x:ClientData ObjectType="Pict">
     <x:CF>Bitmap</x:CF>
     <x:AutoPict/>
...
" o:insetmode="auto">
    <v:imagedata src="rpt_Outbound_files/OUTBOUND_14562_image004.png" o:title=""/>
    <o:lock v:ext="edit" aspectratio="f"/>
    <x:ClientData ObjectType="Pict">
     <x:CF>Bitmap</x:CF>
     <x:AutoPict/>
  ...
  <table cellpadding=0 cellspacing=0>
   ...
   <tr>
    <td height=17></td>
    <td colspan=3 align=left valign=top><img width=895 height=17
    src="images/hor_bar.png"></td>
   </tr>
  </table>
  </span><![endif]><span style='mso-ignore:vglayout2'>
  <table cellpadding=0 cellspacing=0>
   <tr>
    <td height=93 class=xl318414562 width=44 style='height:70.15pt;width:33pt'><a
    name="RANGE!A1:K26"></a></td>
   </tr>
  </table>
  </span></td>
 ...
 <?php
	...
	$totalPCS=0;
	$oldId="";
	$total_PCS_Pack=0;$total_PCS_Carton=0;$total_pcsP=0;$totalPcsPE_Carton=0;$totalpcsPE=0;
	...
	{		
		...
		// Total packing
		// $total_PCS_Pack+=$rowPack["PackNo"];
		
		//-------------------------------------
		// PCS_Carton
		$stPcsC="SELECT COUNT(v.ID_packingDetail) AS PCS_Carton FROM vwPacking_Report v ";
		$stPcsC.=" WHERE v.CusomterOrderNo='".$rowCode["ID_Order"]."' AND v.Code='".$Code."' AND v.PackingType='Carton' GROUP BY v.Code";
		$dtPcsC=mysqli_query($cnn,$stPcsC);
		$rowPcsC=mysqli_fetch_assoc($dtPcsC);	
		// total PCS bocxep carton
		// $total_PCS_Carton+=$rowPcsC["PCS_Carton"];
		//--------------------------------------
		$pcsP=$rowPack["PackNo"]-$rowPcsC["PCS_Carton"];
		// total PCS bocxep pallet
		// $total_pcsP+=$pcsP;
		//------------------------------------
		// PCS_PE
		$stPcsPE="SELECT COUNT(v.ID_packingDetail) AS 'PCS_PE' FROM vwPacking_Report v";
		$stPcsPE.=" WHERE v.CusomterOrderNo='".$rowCode["ID_Order"]."' AND v.Code='".$Code."' AND v.PackingType='Carton' AND v.PE_film_Cover=1 GROUP BY v.Code;";

		$dtPcsPE=mysqli_query($cnn,$stPcsPE);
		$rowPcsPE=mysqli_fetch_assoc($dtPcsPE);	
		// total PCS PE carton
		//$totalPcsPE_Carton+=$rowPcsPE["PCS_PE"];
		//----------------------
		$stPcsPE_2="SELECT COUNT(v.ID_packingDetail) AS 'PCS_PE2' FROM vwPacking_Report v";
		$stPcsPE_2.=" WHERE v.CusomterOrderNo='".$rowCode["ID_Order"]."' AND v.Code='".$Code."' AND v.PackingType<>'Carton' AND v.PE_film_Cover=1 GROUP BY v.Code;";
		$dtPcsPE_2=mysqli_query($cnn,$stPcsPE_2);
		$rowPcsPE_2=mysqli_fetch_assoc($dtPcsPE_2);		
		$pcsPE=$rowPcsPE_2["PCS_PE2"];		
		//******************************************************************************************
		// total PCS PE		
		// $totalpcsPE+=$pcsPE;
		// DIM
		$stDIM="SELECT v.Dim FROM vwPacking_Report v ";
		$stDIM.=" WHERE v.CusomterOrderNo='".$rowCode["ID_Order"]."' AND v.Code='".$Code."'";

		$dtDIM=mysqli_query($cnn,$stDIM);
		$dim="";
		while($rowDIM=mysqli_fetch_assoc($dtDIM))
		{
			$dim.=$rowDIM["Dim"]."<br>";
		}
...
//		}		
 ...
 <tr class=xl317014562 height=26 style='mso-height-source:userset;height:20.1pt'>
  ...
  width:67pt;padding-left:3px; padding-right:3px'><?php echo $Code;?></td>
  ...
  <td class="xl322314562 xl3223145620" style='border-left:none'>
  <input type="text" size="3" style="text-align: center; border: 0px; font-family: "Times New Roman", Times, serif;" value="<?php
  		$myPackNo=$rowPack["PackNo"];		
		if($rowCode["ID_Order"]==$oldId)
		{		
			$myPackNo="";			
  ...
  <td class=xl322314562 width=51 style='border-top:none;border-left:none;
  width:38pt'><?php //echo $rowPcsC["PCS_Carton"]  ?>
  <?php
  		$c=$rowPcsC["PCS_Carton"];		
		if($rowCode["ID_Order"]==$oldId)
		{		
			$c="";			
		}
  ...
  <td class=xl321314562 style='border-top:none;border-left:none'><?php //if($pcsP!=0){ echo $pcsP;}?>
  ...
  <td class=xl321314562 style='border-top:none;border-left:none'><?php //if($rowPcsPE["PCS_PE"]!=0){echo $rowPcsPE["PCS_PE"];}?>
   <?php
  		//$myPackNo=$rowPack["PackNo"];
		$a=$rowPcsPE["PCS_PE"];		
		if($rowCode["ID_Order"]==$oldId)
  ...
  <td class=xl321314562 style='border-top:none;border-left:none'><?php //if($pcsPE!=0){echo $pcsPE;}?>
   <?php
  		//$myPackNo=$rowPack["PackNo"];
		//$a=$rowPcsPE["PCS_PE"];		
		if($rowCode["ID_Order"]==$oldId)
  ...
  <td class=xl322414562a style='border-left:none'><?php //echo $dim;?>
<?php
  		//$myPackNo=$rowPack["PackNo"];
		//$a=$rowPcsPE["PCS_PE"];		
		if($rowCode["ID_Order"]==$oldId)
...
?>
 ...
 <tr height=32 style='mso-height-source:userset;height:24.6pt'>
  ...
  <td class=xl320914562 width=184 style='border-top:none;border-left:none;
  width:138pt'>&nbsp;</td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($totalPCS!=0){ echo $totalPCS;}?></td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($total_PCS_Pack!=0){ echo $total_PCS_Pack;}?></td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($total_PCS_Carton!=0){ echo $total_PCS_Carton;}?></td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($total_pcsP!=0){ echo $total_pcsP;}?></td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($totalPcsPE_Carton!=0){ echo $totalPcsPE_Carton;}?></td>
  <td class=xl321214562 style='border-top:none;border-left:none'><?php if($totalpcsPE!=0){ echo $totalpcsPE;}?></td>
 ...
 <tr height=25 style='mso-height-source:userset;height:18.75pt'>
  <td height=25 class=xl320414562 colspan=11 style='height:18.75pt; white-space:normal'>S&#7889;
