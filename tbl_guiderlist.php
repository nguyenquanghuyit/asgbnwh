...
$sqlG= $sqlG. " WHERE v1.code='".RemoveHtml($tbl_guider->Code->EditValue)."' AND v1.Note_PalletID='".RemoveHtml($tbl_guider->PalletID->EditValue)."' AND v1.Location='".RemoveHtml($tbl_guider->ID_Location->EditValue)."'";
...
if(isset($rowG["pcsG"]))
	...
	$pcsG=RemoveHtml($tbl_guider->PCS_In->EditValue);
?>	
	<?php if ($tbl_guider->PCS->Visible) { // PCS ?>
...
<?php if($pcsG<=0)
{
	echo 0;?>style="color:red" readonly<?php
} 
?>
 class="pcsOut" onChange="tinhTong('<?php echo $tbl_guider->Code->EditValue ?>')" type="text" data-table="tbl_guider" data-field="x_PCS" name="x<?php echo $tbl_guider_list->RowIndex ?>_PCS" id="x<?php echo $tbl_guider_list->RowIndex ?>_PCS" size="3" placeholder="<?php echo HtmlEncode($tbl_guider->PCS->getPlaceHolder()) ?>" 
value="<?php if($pcsG<=0)
{
	echo $tbl_guider->PCS->EditValue;	
}
else
{
	echo $tbl_guider->PCS->EditValue;
...
?>"<?php echo $tbl_guider->PCS->editAttributes() ?>>
</span>
<?php } ?>
<?php if ($tbl_guider->RowType == ROWTYPE_VIEW) { // View record ?>
...
<script src="phpjs\guider_sys.js"></script>
<?php
// Close recordset
